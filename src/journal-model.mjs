...
export function validateEvent(value) {
  ...
  return {id:value.id,text:value.text.trim().replace(/\r\n?/g,'\n'),occurredAt:new Date(value.occurredAt).toISOString(),lat,lon};
}
...
export function exportJournal(rows,format) {
  ...
  return '\uFEFF'+['Id;Datum och tid (UTC);Latitud;Longitud;Händelse',...events.map(e=>[e.id,e.occurredAt,e.lat,e.lon,e.text].map(cell).join(';'))].join('\r\n')+'\r\n';
}
