...
export function mountJournal(api,getPosition) {
  ...
  dialog.innerHTML=`<header class="journal-heading"><h2>Observationsdagbok</h2><button type="button" data-close>Stäng</button></header>
    ...
    <button type="button" data-position>Använd aktuell GPS-position</button><div class="journal-toolbar"><button type="submit" class="button">Spara händelse</button><button type="button" data-delete hidden>Ta bort händelse</button></div></form>
  ...
  let rows=[],selected=null,dirty=false,busy=false,draftId=crypto.randomUUID();
  ...
  function select(row){
    ...
    field('text').value=row?.event.text??'';field('lat').value=row?.event.lat??'';field('lon').value=row?.event.lon??'';
  ...
  function render(){
    ...
    for(const row of [...rows].sort((a,b)=>b.event.occurredAt.localeCompare(a.event.occurredAt))){
      ...
      if(row.conflict){
        const warning=document.createElement('p');warning.textContent='Synkkonflikt. Webbversion: '+(row.conflict.deleted?'Borttagen':new Date(row.conflict.event.occurredAt).toLocaleString('sv-SE')+' · '+row.conflict.event.text);item.append(warning);
  ...
  async function reload(){const data=await api.journalList();rows=data.rows;render();}
  ...
  host.querySelector('[data-open]').onclick=()=>{dialog.showModal();select(null);status.textContent='';$('[data-location]').textContent=api.journalSync?'Sparas på datorn. Synka för att skicka och hämta händelser med din personliga webbnyckel.':'Sparas direkt i din personliga dagbok på webben. Internet krävs.';void run(reload);};
  ...
  form.onsubmit=event=>{event.preventDefault();void run(async()=>{
    const data={id:draftId,occurredAt:new Date(field('occurredAt').value).toISOString(),text:field('text').value,lat:field('lat').value===''?null:Number(field('lat').value),lon:field('lon').value===''?null:Number(field('lon').value)};
